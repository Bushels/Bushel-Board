-- Include admitted Canada cropland/surface-soil moisture proxy rows in Canada thesis packets.
--
-- Saskatchewan reports cropland topsoil moisture and Alberta reports surface-soil
-- moisture as official provincial crop-environment facts. They remain proxy
-- weather evidence in the TypeScript mapper and do not create crop-specific
-- condition, quality, or yield claims.
...
COMMENT ON FUNCTION public.get_canada_thesis_packet(text, text, integer) IS
  'Facts-only Canada source packet for a grain/crop_year/grain_week. Separates source facts from AI interpretation. Supply rows prefer the requested packet crop year and include latest province-level seeded, crop-condition, bounded crop-development timing, and bounded cropland/surface-soil moisture rows where mapped.';
