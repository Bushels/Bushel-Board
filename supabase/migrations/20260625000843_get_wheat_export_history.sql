-- Public-safe Wheat export demand history for the farmer-facing thesis board.
--
-- Also repairs the Canada thesis packet export formula so it matches the
-- documented CGC composite export definition:
--   Terminal Exports + Primary Shipment Distribution Export Destinations
--   + Producer Cars Shipment Distribution Export
--
-- Terminal Disposition.Export Destinations cross-checks terminal exports; it
-- must not be added on top of Terminal Exports or the packet double-counts
-- the port movement.
...
$new$;
BEGIN
  SELECT pg_get_functiondef('public.get_canada_thesis_packet(text,text,integer)'::regprocedure)
  INTO fn;

  IF fn IS NULL THEN
    RAISE EXCEPTION 'get_canada_thesis_packet(text,text,integer) not found';
  END IF;

  IF position(old_fragment IN fn) = 0 THEN
    RAISE EXCEPTION 'Expected CGC export fragment not found in get_canada_thesis_packet';
  END IF;

  EXECUTE replace(fn, old_fragment, new_fragment);
END $$;

GRANT EXECUTE ON FUNCTION public.get_canada_thesis_packet(text, text, integer)
  TO anon, authenticated;

COMMENT ON FUNCTION public.get_canada_thesis_packet(text, text, integer) IS
  'Facts-only Canada source packet for a grain/crop_year/grain_week. Separates source facts from AI interpretation. Canada exports use the documented CGC composite formula: Terminal Exports + Primary Shipment Distribution Export Destinations + Producer Cars Shipment Distribution Export.';
...
AS $$
  ...
  source_freshness AS (
    SELECT
      (
        SELECT f.freshness_status
        FROM public.get_thesis_data_freshness('Wheat', NULL) f
        WHERE f.source_name = 'cgc_observations'
        LIMIT 1
      ) AS cgc_status,
      (
        SELECT f.freshness_status
        FROM public.get_thesis_data_freshness('Wheat', NULL) f
        WHERE f.source_name = 'usda_export_sales'
        LIMIT 1
      ) AS usda_status,
      (
        SELECT max(c.week_ending_date)::date
        FROM public.cgc_observations c
        WHERE lower(c.grain) = 'wheat'
      ) AS cgc_period_end,
      (
        SELECT max(e.week_ending)::date
        FROM public.usda_export_sales e
        WHERE e.commodity = 'ALL WHEAT'
          OR lower(e.cgc_grain) = 'wheat'
      ) AS usda_period_end
  ),
  canada_weeks AS (
    SELECT w.crop_year, w.grain_week, w.week_ending_date
    FROM (
      SELECT DISTINCT c.crop_year, c.grain_week, c.week_ending_date
      FROM public.cgc_observations c
      WHERE lower(c.grain) = 'wheat'
    ) w
    ORDER BY w.week_ending_date DESC, w.grain_week DESC
  ...
  canada_deliveries AS (
    SELECT
      v.crop_year,
      v.grain_week,
      max(v.total_kt) FILTER (WHERE v.period = 'Current Week') AS weekly_producer_deliveries_kt,
      max(v.total_kt) FILTER (WHERE v.period = 'Crop Year') AS cumulative_producer_deliveries_kt
    FROM public.v_country_producer_deliveries v
    JOIN canada_weeks w
      ON w.crop_year = v.crop_year
      AND w.grain_week = v.grain_week
    WHERE lower(v.grain) = 'wheat'
    GROUP BY v.crop_year, v.grain_week
  ),
  canada_rows AS (
    SELECT
