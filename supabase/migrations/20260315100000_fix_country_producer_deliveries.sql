-- =============================================================================
-- Standardize producer delivery math across dashboard views and RPCs
-- Formula: Primary.Deliveries (AB/SK/MB/BC) + Process.Producer Deliveries
--        + Producer Cars.Shipments (AB/SK/MB)
-- =============================================================================

CREATE OR REPLACE VIEW public.v_country_producer_deliveries AS
WITH normalized AS (
  SELECT
    o.grain,
    o.crop_year,
  ...
  WHERE o.worksheet = 'Primary'
    AND o.metric = 'Deliveries'
    AND o.grade = ''
    AND o.region IN ('Alberta', 'Saskatchewan', 'Manitoba', 'British Columbia')
  GROUP BY o.grain, o.crop_year, o.grain_week, o.week_ending_date, o.period
  ...
  SELECT
    ...
    o.week_ending_date,
    o.period,
    'process'::text AS source,
    SUM(o.ktonnes) AS kt
  FROM public.cgc_observations o
  WHERE o.worksheet = 'Process'
  ...
  SELECT
    ...
    o.week_ending_date,
    o.period,
    'producer_cars'::text AS source,
    SUM(o.ktonnes) AS kt
  FROM public.cgc_observations o
  WHERE o.worksheet = 'Producer Cars'
    AND o.metric = 'Shipments'
    AND o.grade = ''
...
SELECT
  ...
  n.crop_year,
  n.grain_week,
  MAX(n.week_ending_date) AS week_ending_date,
  n.period,
  COALESCE(SUM(n.kt) FILTER (WHERE n.source = 'primary'), 0) AS primary_deliveries_kt,
  COALESCE(SUM(n.kt) FILTER (WHERE n.source = 'process'), 0) AS process_deliveries_kt,
  COALESCE(SUM(n.kt) FILTER (WHERE n.source = 'producer_cars'), 0) AS producer_cars_kt,
  COALESCE(SUM(n.kt), 0) AS total_kt
FROM normalized n
GROUP BY n.grain, n.crop_year, n.grain_week, n.period;

COMMENT ON VIEW public.v_country_producer_deliveries IS
  'Country-level producer deliveries by grain/week/period: Primary deliveries (AB/SK/MB/BC) + Process producer deliveries + Producer Cars shipments.';

CREATE OR REPLACE VIEW public.v_grain_overview AS
WITH latest_week AS (
...
cy_deliveries AS (
  SELECT vcpd.grain, vcpd.total_kt AS cy_deliveries_kt
  FROM public.v_country_producer_deliveries vcpd
  CROSS JOIN latest_week lw
  WHERE vcpd.crop_year = lw.crop_year
    AND vcpd.grain_week = lw.max_week
    AND vcpd.period = 'Crop Year'
),
cw_deliveries AS (
  SELECT vcpd.grain, vcpd.total_kt AS cw_deliveries_kt
  FROM public.v_country_producer_deliveries vcpd
  CROSS JOIN latest_week lw
  WHERE vcpd.crop_year = lw.crop_year
    AND vcpd.grain_week = lw.max_week
...
prev_deliveries AS (
  SELECT vcpd.grain, vcpd.total_kt AS prev_deliveries_kt
  FROM public.v_country_producer_deliveries vcpd
  CROSS JOIN latest_week lw
  WHERE vcpd.crop_year = lw.crop_year
    AND vcpd.grain_week = (lw.max_week - 1)
    AND vcpd.period = 'Current Week'
)
SELECT
  g.name AS grain,
  ...
  CASE
    WHEN COALESCE(prev.prev_deliveries_kt, 0::numeric) > 0
      THEN ROUND(((COALESCE(cw.cw_deliveries_kt, 0::numeric) - prev.prev_deliveries_kt) / prev.prev_deliveries_kt * 100)::numeric, 1)
    ELSE 0::numeric
...
current_stocks AS (
  ...
  WHERE o.crop_year = l.crop_year
    ...
    AND o.period = 'Current Week'
    AND o.grade = ''
    AND o.region IN ('Primary Elevators', 'Process Elevators')
  GROUP BY o.grain
),
prior_stocks AS (
...
CREATE OR REPLACE FUNCTION public.get_historical_average(
  p_grain text,
  p_metric text,
  p_worksheet text,
...
AS $$
  ...
  delivery_values AS (
    SELECT vcpd.crop_year, vcpd.total_kt
    FROM public.v_country_producer_deliveries vcpd, current_cy c
    WHERE p_metric = 'Deliveries'
      AND p_worksheet = 'Primary'
      AND vcpd.grain = p_grain
      AND vcpd.grain_week = p_grain_week
      AND vcpd.period = 'Crop Year'
      AND vcpd.crop_year != c.crop_year
    ORDER BY vcpd.crop_year DESC
    LIMIT p_years_back
  ),
  other_values AS (
    SELECT o.crop_year, SUM(o.ktonnes) AS total_kt
    FROM public.cgc_observations o, current_cy c
    WHERE NOT (p_metric = 'Deliveries' AND p_worksheet = 'Primary')
      AND o.grain = p_grain
...
CREATE OR REPLACE FUNCTION public.get_week_percentile(
  p_grain text,
  p_metric text,
  p_worksheet text,
  p_grain_week integer,
...
GRANT EXECUTE ON FUNCTION public.get_historical_average(text, text, text, integer, integer) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.get_week_percentile(text, text, text, integer, numeric, integer) TO authenticated, service_role;
