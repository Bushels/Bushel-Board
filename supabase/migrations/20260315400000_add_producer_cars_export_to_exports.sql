-- =============================================================================
-- Add Producer Cars Export component to CGC Exports formula
--
-- CGC Summary "Exports" = 3 components (verified against Excel):
--   1. Terminal Exports (vessels leaving ports)
--   2. Primary Shipment Distribution "Export Destinations" (direct from elevators)
--   3. Producer Cars Shipment Distribution "Export" (farmer railcars direct to US)
--
-- Verification (Canola Week 31):
--   Terminal Exports: 112.5 + PSD Export Destinations: 0.1 + PC Export: 0.9 = 113.5
--   CGC Excel Summary!H27 = 113.5 ✓
--
-- Verification (Wheat Week 31):
--   Terminal Exports: 420.7 + PSD Export Destinations: 21.4 + PC Export: 0.0 = 442.1
--   CGC Excel Summary!B27 = 442.1 ✓
--
-- Affected objects:
...
AS $$
  ...
  exports AS (
    -- CGC "Exports" = Terminal Exports + PSD Export Destinations + Producer Cars Export
    SELECT o.grain_week, SUM(o.ktonnes) AS kt
    FROM public.cgc_observations o
    WHERE o.crop_year = p_crop_year
      AND o.grain = p_grain
...
-- 3. Fix v_grain_yoy_comparison: add Producer Cars Export to both current and prior year
...
-- Total commercial stocks = ALL Summary Stocks regions
current_stocks AS (
...
-- Prior year exports also needs all 3 components
prior_exports AS (
