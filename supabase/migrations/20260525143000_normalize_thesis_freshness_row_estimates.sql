-- Track B source-health truth pass: do not let stale pg_class row estimates
-- turn source rows with valid latest periods into false empty blockers. This is
-- especially visible on usda_wasde_mapped after raw WASDE/import cache refreshes.
  ...
  source_status AS (
    SELECT
      ...
      CASE
        ...
        WHEN s.rows_available = 0 AND s.latest_period_end IS NULL THEN 'empty'
      ...
      CASE
        ...
        WHEN s.rows_available = 0 AND s.latest_period_end IS NULL THEN 'Build or seed this source before thesis use.'
...
COMMENT ON FUNCTION public.get_thesis_data_freshness(text, text) IS
  'Fast source freshness summary for an optional canonical grain and lane, including Canada crop progress, USDA quarterly stocks, and acreage. Uses latest-row lookups plus estimated row counts; latest dated source rows take precedence over stale zero row-count estimates so source health does not produce false empty blockers.';
