-- Keep the public Wheat export-history RPC fast enough for PostgREST.
-- The first version reused get_thesis_data_freshness(), which is too heavy for
-- a small chart RPC and can hit the REST statement timeout. This version keeps
-- the same public return shape but computes freshness directly from source
-- period dates.
