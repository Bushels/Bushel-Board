-- 1. Fix get_pipeline_velocity: add Producer Cars Export
...
-- 2. Fix get_pipeline_velocity_avg: add Producer Cars Export
...
-- 3. Fix v_grain_yoy_comparison view
...
WITH latest AS (
  ...
  WHERE crop_year = (
    SELECT crop_year FROM public.cgc_observations ORDER BY crop_year DESC LIMIT 1
  )
  GROUP BY crop_year
),
prior_year AS (
  ...
  ORDER BY crop_year DESC LIMIT 1
),
current_deliveries AS (
  ...
  WHERE vcpd.crop_year = l.crop_year AND vcpd.grain_week = l.max_week AND vcpd.period = 'Crop Year'
),
current_week_deliveries AS (
  SELECT vcpd.grain, vcpd.total_kt AS cw_deliveries
  FROM public.v_country_producer_deliveries vcpd, latest l
  WHERE vcpd.crop_year = l.crop_year AND vcpd.grain_week = l.max_week AND vcpd.period = 'Current Week'
),
prior_week_deliveries AS (
  SELECT vcpd.grain, vcpd.total_kt AS pw_deliveries
  FROM public.v_country_producer_deliveries vcpd, latest l
  WHERE vcpd.crop_year = l.crop_year AND vcpd.grain_week = (l.max_week - 1) AND vcpd.period = 'Current Week'
),
current_terminal_receipts AS (
  ...
  WHERE o.crop_year = l.crop_year AND o.grain_week = l.max_week
    AND o.worksheet = 'Terminal Receipts' AND o.metric = 'Receipts' AND o.period = 'Current Week'
  GROUP BY o.grain
),
cy_terminal_receipts AS (
  ...
  FROM public.cgc_observations o, latest l
  WHERE o.crop_year = l.crop_year AND o.grain_week = l.max_week
    AND o.worksheet = 'Terminal Receipts' AND o.metric = 'Receipts' AND o.period = 'Crop Year'
  GROUP BY o.grain
),
...
current_exports AS (
  ...
  FROM public.cgc_observations o, latest l
  WHERE o.crop_year = l.crop_year AND o.grain_week = l.max_week AND o.period = 'Crop Year'
    AND (
      (o.worksheet = 'Terminal Exports' AND o.metric = 'Exports')
      OR (o.worksheet = 'Primary Shipment Distribution' AND o.metric = 'Shipment Distribution' AND o.region = 'Export Destinations')
      OR (o.worksheet = 'Producer Cars' AND o.metric = 'Shipment Distribution' AND o.region = 'Export')
    )
  GROUP BY o.grain
),
current_crush AS (
  ...
  FROM public.cgc_observations o, latest l
  WHERE o.crop_year = l.crop_year AND o.grain_week = l.max_week
    AND o.worksheet = 'Process' AND o.metric = 'Milled/Mfg Grain' AND o.period = 'Crop Year' AND o.region = ''
  GROUP BY o.grain
),
current_stocks AS (
  ...
  FROM public.cgc_observations o, latest l
  WHERE o.crop_year = l.crop_year AND o.grain_week = l.max_week
    AND o.worksheet = 'Summary' AND o.metric = 'Stocks' AND o.period = 'Current Week' AND o.grade = ''
  GROUP BY o.grain
),
prior_stocks AS (
...
prior_deliveries AS (
  ...
  WHERE vcpd.crop_year = py.crop_year AND vcpd.grain_week = l.max_week AND vcpd.period = 'Crop Year'
),
prior_exports AS (
  SELECT o.grain, SUM(o.ktonnes) AS py_exports
  FROM public.cgc_observations o, latest l, prior_year py
  WHERE o.crop_year = py.crop_year AND o.grain_week = l.max_week AND o.period = 'Crop Year'
    AND (
      (o.worksheet = 'Terminal Exports' AND o.metric = 'Exports')
      OR (o.worksheet = 'Primary Shipment Distribution' AND o.metric = 'Shipment Distribution' AND o.region = 'Export Destinations')
      OR (o.worksheet = 'Producer Cars' AND o.metric = 'Shipment Distribution' AND o.region = 'Export')
