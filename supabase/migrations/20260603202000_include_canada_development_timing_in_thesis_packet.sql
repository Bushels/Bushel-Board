-- Include admitted Canada crop-development timing rows in Canada thesis packets.
--
-- Saskatchewan development rows are official provincial crop-group timing facts.
-- They remain proxy/group context in the TypeScript mapper, but the packet must
-- carry them before the mapper can apply a bounded timing signal.
...
COMMENT ON FUNCTION public.get_canada_thesis_packet(text, text, integer) IS
  'Facts-only Canada source packet for a grain/crop_year/grain_week. Separates source facts from AI interpretation. Supply rows prefer the requested packet crop year and include latest province-level seeded, crop-condition, and bounded crop-development timing rows where mapped.';
