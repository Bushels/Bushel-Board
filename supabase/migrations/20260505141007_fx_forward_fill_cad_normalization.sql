-- Use latest available USD/CAD rate on or before the futures price date.
--
-- Yahoo FX data can skip weekends/holidays or publish on a different calendar
-- than futures settlements. Exact same-day matching leaves usable USD futures
-- without CAD normalization, so the deterministic fallback is previous
-- available USD/CAD.
