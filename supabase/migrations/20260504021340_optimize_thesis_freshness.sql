-- Data Layer Foundation V1 follow-up:
-- Optimize the app-facing freshness RPC. The original view is useful as a
-- readable source map, but the RPC timed out through PostgREST because it did
-- exact count(*) scans across large source tables on every call.

CREATE OR REPLACE FUNCTION public.estimated_table_rows(p_table regclass)
RETURNS bigint
LANGUAGE sql
STABLE
SECURITY INVOKER
...
CREATE INDEX IF NOT EXISTS idx_cgc_observations_latest_week
  ON public.cgc_observations (crop_year DESC, grain_week DESC);

CREATE INDEX IF NOT EXISTS idx_cgc_imports_latest
  ON public.cgc_imports (imported_at DESC);

CREATE INDEX IF NOT EXISTS idx_supply_disposition_latest
  ON public.supply_disposition (crop_year DESC, created_at DESC);

CREATE INDEX IF NOT EXISTS idx_usda_export_sales_latest
  ON public.usda_export_sales (week_ending DESC, imported_at DESC);

CREATE INDEX IF NOT EXISTS idx_cftc_cot_latest
  ON public.cftc_cot_positions (report_date DESC, imported_at DESC);
...
COMMENT ON FUNCTION public.get_thesis_data_freshness(text, text) IS
  'Fast source freshness summary for an optional canonical grain and lane. Uses latest-row lookups plus estimated row counts so PostgREST calls do not time out.';
