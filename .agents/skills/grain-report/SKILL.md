...
- Wheat export-demand history now uses bounded public RPC `get_wheat_export_history(weeks)`; raw `usda_export_sales` remains non-public. CGC export math is Terminal Exports + Primary Shipment Distribution Export Destinations + Producer Cars Shipment Distribution Export. Do not add Terminal Disposition Export Destinations because it double-counts terminal exports. After the 2026-06-24 repair, cached week-45 Canada Wheat exports are 193.9 kt current week and 19,663.9 kt crop year, with a 30.2% export/delivery ratio.
- A same-period published `market_analysis` or `us_market_analysis` desk row is the farmer-facing weekly thesis-of-record on `/thesis`. The deterministic packet scorecard remains the mechanical evidence cross-check and fallback when no matching published row exists; a reviewed daily overlay remains the only current-day override.
- `/thesis` has an evidence cross-check with deciding-datum freshness, counterweight language, and source-specific copy; a USDA Wheat source-sweep panel for Crop Progress/WASDE/Export Sales/Quarterly Stocks with decision roles and a condition-to-price relationship chain; a node-and-edge relationship spiderweb; and a Spring Wheat / HRW / SRW price-basket proof strip. These explain the mechanical scorecard beneath the published desk read; they do not replace the published thesis.
...
3. Start from the visible USDA Wheat source-sweep panel and the price/export context panels, then inventory source rows before changing weights: USDA Crop Progress, WASDE, Export Sales, Quarterly Stocks, CGC flow, Grain Monitor, producer cars, CFTC COT, prices/FX, and Hermes/X pulse artifacts.
