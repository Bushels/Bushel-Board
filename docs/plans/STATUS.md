...
| 50 | Kalshi Live Wiring on Overview Marketplace | Complete | 2026-04-28 | Replaces the Track 49 mock Kalshi cards with live data from the Kalshi public API (`https://api.elections.kalshi.com/trade-api/v2`, read-only, no auth). **Coverage audit (2026-04-28):** Kalshi's grain-commodity universe is exactly 6 binary price contracts (corn/soy/wheat × weekly/monthly). All 6 surfaced + KXFERT (fertilizer year-end strike, $9.9k+ volume) as the input-cost wildcard = **7 cards total** organized into two cadence rows ("Monthly contracts · Inputs" — 4 cards; "Weekly contracts" — 3 cards). New: `lib/kalshi/types.ts` (shared `KalshiMarket`/`KalshiCadence`/`KalshiSeriesSpec`/`KalshiRawMarket` shapes with isolation-fence header), `lib/kalshi/client.ts` (server-only client: `fetchKalshiMarkets()` orchestrator with **120ms request stagger to dodge Kalshi rate-limits** + 5-min in-memory cache, `fetchTopMarketForSeries()` per-series picker with **single retry on 429/5xx**, parse/normalize helpers, `deriveDisplayTitle()` rewrites generic KXFERT titles to include the strike level, identifying User-Agent, graceful `[]`-on-failure semantics, tunable `FEATURED_KALSHI_TICKERS` const = KXCORNMON · KXSOYBEANMON · KXWHEATMON · KXFERT (wildcard) · KXCORNW · KXSOYBEANW · KXWHEATW). Modified: `components/overview/marketplace-strip.tsx` is now an async server component rendering 7 live market cards split into two cadence-grouped rows with row sublabels, soil-tone (`#6b3f2a`) accent for fertilizer cards, "Live · Kalshi + CBOT" header + "As of {time} · Kalshi public API" footnote, and a static 7-card snapshot fallback (with "Live data unavailable" note) when the API is unreachable. Tests: `lib/__tests__/kalshi-client.test.ts` (35 tests covering parse/format/normalize helpers, deriveDisplayTitle for KXFERT title rewriting, fetchTopMarketForSeries 200/500-with-retry/429-retry-success/throw/malformed-JSON/empty paths, fetchKalshiMarkets parallelism + partial failure + cache + 7-ticker default contract + cadence carry-through). Verified live: smoke fetch returned all 7 markets — corn 66% mon / soy 89% mon / wheat 91% mon / fert 51% wildcard / corn 52% wk / soy 96% wk / wheat 88% wk; browser DOM eval on `/overview` confirmed `cardCount: 7`, both row sublabels rendering, FERTILIZER label with soil tone, asOf timestamp present. **Isolation fences** documented in all 3 Kalshi files — module is structurally separate from grain-desk / market_analysis / score_trajectory pipelines and must stay that way until a deliberate richer integration is designed. |
| 51 | Predictive Market — Editorial Trading Floor redesign | Complete | 2026-04-29 | Promotes the 7-card grid (Track 50) into a premium "Editorial Trading Floor" surface that reads like a grain-belt market broadsheet with a live trading-desk pulse. Replaces flat cards with a four-zone composition: (1) **PredictiveMarketHeader** — italic-Prairie "Predictive Market" title, lede, and a collapsible `<details>` "What is a predictive market?" disclosure (server-renderable, no JS needed); (2) **LiveTape** — `"use client"` always-scrolling marquee at 60s loop, polls `/api/kalshi/trades?ticker=...` every 12 seconds, seeded server-side so first paint is non-empty, respects `prefers-reduced-motion`; (3) **SpotlightCard** — hero card with the highest-volume market: clamped 56→96px Fraunces YES probability + 24h movement label + stroke-drawn SVG sparkline from real Kalshi candlesticks (Catmull-Rom mid-point smoothing, gradient fill, terminal dot) + 5-row recent-prints rail + "Place a bet on Kalshi" primary CTA; (4) **MarketRoll** — dense GrainStanceGrid-style table of the remaining 6 markets (crop pill · cadence short · question · mini-spark from bid/last/ask · YES% · 24h delta · scaled volume bar · → Kalshi link). Below: existing **CBOTSpotTiles** restyled as "CBOT Futures · Ground truth" tier with editorial sub-label. New `lib/kalshi/` extensions: `KalshiCandle`/`KalshiTrade`/`KalshiSpotlightExtras`/`KalshiRawCandle`/`KalshiRawTrade` types, `fetchCandlesticks()` + `fetchRecentTrades()` + `pickSpotlightMarket()` + `normalizeKalshiCandle()` + `normalizeKalshiTrade()` + `candleMidPrice()` helpers, all behind the same isolation fences and graceful `[]`-on-failure semantics. New API route `app/api/kalshi/trades/route.ts` (thin server-side proxy for the LiveTape polling, validated ticker pattern, no-store cache). Tunable rate-limit constants bumped: `KALSHI_STAGGER_MS` 120 → 250 ms (4 req/sec ceiling clears Kalshi's public cliff comfortably), spotlight extras now sequenced (250ms breather then candles → 250ms → trades) so the 9-call sequence per cold render never lands above 4/sec. Tests: 51 total in `lib/__tests__/kalshi-client.test.ts` (16 new for candle normalize / mid-price / trade normalize / spotlight picker happy + degenerate paths). Verified live (browser DOM eval, fresh SSR): `cardCount: 7`, sparkline `aria-label="Sparkline trending up"`, tape rendered with FERT prints + age labels, spotlight reads `51¢ YES · ↑ 5 cents in 24h · NO 49¢ · Place a bet on Kalshi`, roll has 6 rows (`CORN MON · Will the corn close price be above $455.99 on Apr 30 · 66¢ · ↓6¢ · $3.0k`), `/api/kalshi/trades` proxy returns 3 normalized trades. New components colocated under `components/overview/marketplace/`: `sparkline.tsx`, `spotlight-card.tsx`, `market-roll.tsx`, `live-tape.tsx`, `predictive-market-header.tsx`. Single integration point (`components/overview/marketplace-strip.tsx`) labeled as the only bridge between the isolated Kalshi module and `/overview`. Dispatched via the `frontend-design:frontend-design` skill with explicit "Editorial Trading Floor" brief. |
