...
| 51 | Bull/Bear Thesis Board V1 | Complete | 2026-05-08 | `app/(dashboard)/thesis/page.tsx`, `lib/queries/thesis-board.ts`, `lib/__tests__/thesis-board.test.ts`, `supabase/migrations/20260509033204_add_thesis_packet_cache.sql`, `scripts/refresh-thesis-packet-cache.ts`, `scripts/run-collector-with-thesis-cache-refresh.ts`, `components/layout/desktop-nav-links.tsx`, `components/layout/mobile-nav.tsx`, `docs/reference/collector-task-configs.md`. Renders Canada and US bull/bear packet cards from `get_thesis_board_cached()`, shows stance/confidence/freshness/provenance, warms 21 cache packets, and wires `npm run collect:*` commands so successful source collectors refresh the thesis cache. |
| 52 | Predictive Market Harness - Canola to Multi-Grain | Phase 2 complete; Phase 3 starting | 2026-05-10 | Canola no-write harness is committed through Patch 2H: local source records, snapshots, prompt packs, manual run artifacts, sidecar packages, outcome scores, thesis review packages, historical replay packages, first real frozen Canola thesis, first CGC replay bundle, and point-in-time CGC snapshot capture. Active Codex automations: `canola-predictive-harness-no-write-review` and `cgc-weekly-point-in-time-snapshot-capture`. Phase 3 handoff: `docs/plans/2026-05-10-multi-grain-weekly-thesis-harness-handoff.md`. Next step is a shared grain-agnostic engine plus canonical 16-grain profiles; no training, sidecar writer, production/dashboard integration, or Hermes automation yet. |
