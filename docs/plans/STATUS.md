...
Current active track note (2026-05-31): Track 51 remains the active product lane. Use `docs/plans/2026-05-31-bushel-board-fresh-session-handoff.md` first. Current verified state: `/thesis` and `/overview` are hard-gated to source-backed V1 public rows only (Corn, Soybeans, Wheat, Durum/Amber Durum, Canola, Barley, Oats); Spring/Winter Wheat are parked off public board surfaces until class-safe source mapping is admitted; live `thesis_packet_cache` is pruned/hard-gated; live `prediction_scorecard` RLS is public read-only with service-role writes retained. Verification on 2026-05-31: focused thesis tests passed, `npm run lint` passed with 0 errors / 68 warnings, `npm run build` passed, local browser DOM probes for `/thesis?verify=1` and `/overview?verify=1` showed no forbidden smaller crops and clean console. Full Vitest all-files run still hangs as test-runner/open-handle debt; isolate that separately before treating it as product regression.
