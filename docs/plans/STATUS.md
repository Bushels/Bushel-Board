...
| 50 | Data Layer Foundation V1 | In progress | 2026-05-06 | Branch `codex/data-layer-foundation-v1`, latest baseline commit `0f7bcdc`. Live Supabase migrations are applied for `source_runs`, grain/market mappings, source freshness, Canada/US thesis packet RPCs, freshness optimization, Canola source-admission automation support, intraday Canola quote storage, FX CAD recalculation grant hardening, and Canola packet supply-year selection. Sprint-1 docs cover source registry, canonical fact model, and Canola Market Read V1. Deterministic Canola read generator now renders from `get_canada_thesis_packet('Canola', ...)`; validator passes for Canola; `/source-spine` leads with the core weekly spine; `source_runs` now has successful live rows for Producer Cars, CFTC COT, StatsCan crop baseline, Canola Council inventory, FX rates, and grain prices. |
