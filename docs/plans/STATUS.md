...
| 50 | Data Layer Foundation V1 | In progress | 2026-05-04 | Branch `codex/data-layer-foundation-v1`, latest pushed commit `2419e6b`. Live Supabase migrations are applied for `source_runs`, grain/market mappings, source freshness, Canada/US thesis packet RPCs, and freshness optimization. Sprint-1 docs cover source registry, canonical fact model, and Canola Market Read V1. Deterministic Canola read generator now renders from `get_canada_thesis_packet('Canola', ...)`; validator passes for Canola; `/source-spine` now leads with the core weekly spine (CGC, Grain Monitor, producer cars / rail staging, COT, AAFC/StatsCan baseline) before watched AAFC/GEE source lanes; `source_runs` is live but empty until collectors write run summaries. |
