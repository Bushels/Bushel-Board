...
| 51 | Bull/Bear Thesis Board V1 | Complete | 2026-05-10 | `app/(dashboard)/thesis/page.tsx`, `lib/queries/thesis-board.ts`, `lib/__tests__/thesis-board.test.ts`, `supabase/migrations/20260509033204_add_thesis_packet_cache.sql`, `scripts/refresh-thesis-packet-cache.ts`, `scripts/run-collector-with-thesis-cache-refresh.ts`, `components/layout/desktop-nav-links.tsx`, `components/layout/mobile-nav.tsx`, `docs/reference/collector-task-configs.md`. Renders facts-only Canada/US thesis packets, then filters V1 to 10 Canada major grains plus 5 US overview markets. The top matrix shows CA/US direction indicators, confidence, strongest bull/bear points, and country-split explanations. Smaller CGC labels and US rice/cotton are excluded from V1. |
| 52 | Predictive Market Harness - Canola to Multi-Grain | Phase 3A complete; paused for Kalshi focus | 2026-05-10 | Canola no-write harness is committed through Patch 2H. Local Phase 3A refactor work adds shared grain profiles and a grain-agnostic harness path while preserving Canola compatibility, but no sidecar writer, production/dashboard write path, training claim, or Hermes automation was added. This track is paused unless explicitly resumed; current product focus is Track 53 Kalshi Prediction Board. |
| 53 | Kalshi Prediction Board - Calibration V1 | Complete with Bushel Board Implied Line | 2026-05-10 | Read-only calibration layer for Kalshi Corn, Soybeans, and Wheat weekly/monthly commodity series. New: `lib/kalshi/commodity-markets.ts`, `lib/queries/kalshi-commodities.ts`, `scripts/capture-kalshi-commodity-snapshot.ts`, `lib/__tests__/kalshi-commodity-markets.test.ts`; `/kalshi` renders equal-weight Kalshi Prediction Board cards and `/overview` links to that live board instead of showing mock Kalshi cards. The cards compare Kalshi YES against a deterministic Bushel Board Implied Line built from existing thesis direction, confidence, and ranked evidence, with delta, bull/bear reasons, spread/liquidity warnings, and no line when the market threshold or thesis evidence is insufficient. 2026-05-11 update: the Kalshi fetch now proves public API wiring by showing latest returned API markets separately when no open markets are available; closed/finalized markets are labeled as wiring proof only and are not used for the implied line. No Supabase writes, no trading path, no thesis-prompt feedback, no scorecard/training writes, and no rice/cotton or smaller-grain expansion. |
