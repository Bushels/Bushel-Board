...
| 50 | Data Layer Foundation V1 | In progress | 2026-05-03 | Branch `codex/data-layer-foundation-v1`, latest pushed commit `cd7bbda`. Live Supabase migrations are applied for `source_runs`, grain/market mappings, source freshness, Canada/US thesis packet RPCs, and freshness optimization. Sprint-1 docs now cover source registry, canonical fact model, and Canola Market Read V1. Validator passes for Canola; `source_runs` is live but empty until collectors write run summaries. |
