...
| 50 | Kalshi Live Wiring on Overview Marketplace | Complete | 2026-04-28 | Replaces the Track 49 mock Kalshi cards with live data from the Kalshi public API (`https://api.elections.kalshi.com/trade-api/v2`, read-only, no auth). New: `lib/kalshi/types.ts` (shared `KalshiMarket`/`KalshiSeriesSpec`/`KalshiRawMarket` shapes), `lib/kalshi/client.ts` (server-only client: `fetchKalshiMarkets()` orchestrator + `fetchTopMarketForSeries()` per-series picker, normalize/parse helpers, in-memory module cache w/ 5-min TTL, identifying User-Agent, graceful `[]`-on-failure semantics, tunable `FEATURED_KALSHI_TICKERS` const = KXCORNMON · KXSOYBEANMON · KXWHEATMON · KXCORNW). Modified: `components/overview/marketplace-strip.tsx` is now an async server component that renders 4 live market cards (real YES probability, volume, close date), shows "Live · Kalshi + CBOT" header + "As of {time} · Kalshi public API" footnote, and falls back to a static snapshot with an explicit "Live data unavailable" note when the API is unreachable. Tests: `lib/__tests__/kalshi-client.test.ts` (30 tests covering parseKalshiNumber/formatCloseLabel/formatVolume/deriveYesProbability/normalizeKalshiMarket happy + degenerate paths, fetchTopMarketForSeries with 200/500/throw/malformed-JSON/empty paths, fetchKalshiMarkets parallelism + partial failure + cache + default-list contract). Verified live: smoke fetch returned corn 66% / soy 91% / wheat 91% / corn-weekly 52% with $1k–$3.8k volumes; browser DOM eval on `/overview` confirmed header + cards + asOf timestamp render with real values. |
