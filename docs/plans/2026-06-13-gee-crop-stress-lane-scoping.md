...
- **P-G-RUSSIA** ✅ **DONE 2026-06-15** — belt registry in `gee_stress_core.py` (`BELTS`, `belt_geometries`, `mask_for_belt`) + `--belt` on the collector. Russia belts use **ESA WorldCover cropland (class 40)** mask (no CDL outside US) and AOI = GAUL country boundary ∩ winter/spring bbox. First readings (wk 2026-06-14): **RU_WINTER stress_index +0.35 (healthy)** — corroborates SovEcon's strong-winter-wheat / raised crop; **RU_SPRING −0.13 (near-normal)**, **US_HRW −0.73 (stressed)**. **CAVEAT (critical): GEE stress = crop CONDITION, not planted AREA.** RU_SPRING near-normal condition does NOT contradict the Sizov structural *acreage* decline — area is a separate signal NDVI cannot see. Russia belts are belt-level only (no per-oblast yet) and still WATCH-ONLY.
- **P-G5** Validate across multiple seasons incl. a reversal (§7) → graduate to weather domain for belts where it backtests. Russia validation has no NASS equivalent — backtest vs SovEcon/USDA-FAS production revisions instead.
