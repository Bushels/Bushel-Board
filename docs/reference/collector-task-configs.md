...
- 2026-05-16 finding: `collect:export-sales` imported current USDA data through `2026-05-07`, but the wrapper-triggered cache refresh skipped until `npm run refresh-thesis-cache -- --force` was run. Next patch should make collector-triggered cache refresh force by default or compare against latest `source_runs` directly.
