...
4. **Never mix scopes** — do not combine national AAFC numbers with prairie-only CGC numbers in the same calculation
5. **`macro_estimates` is DEPRECATED** — do not query or reference this table
...
| CGC cgc_observations | Prairie provinces (AB, SK, MB) | Weekly (Thursday) |
...
```
CGC Weekly CSV → cgc_observations (prairie weekly metrics)
                      ↓
