...
See also `docs/reference/grain-impact-mind-map-v1.md` for grain-specific impact factors, relationships, seasonal gates, and source-gap boundaries.
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `cgc_weekly_stats` | Canada | Canadian Grain Commission Grain Statistics Weekly CSV | `cgc_observations`, `cgc_imports`, `get_wheat_export_history(weeks)` bounded Wheat demand-history RPC | `npm run import-cgc` | Weekly, Thursday after CGC publish | `crop_year`, `grain_week`, `week_ending_date` | `Ktonnes` | CGC publish delay, external fetch block, local CSV cache stale | Canada movement anchor: deliveries, shipments, terminal receipts, exports, stocks. CGC exports are Terminal Exports + Primary Shipment Distribution Export Destinations + Producer Cars Shipment Distribution Export; Terminal Disposition Export Destinations is a cross-check, not an additive export component. |
...
| `canada_crop_progress` | Canada | Manitoba, Saskatchewan, and Alberta official provincial crop reports | `canada_crop_progress` | `python scripts/import-canada-crop-progress.py` | Province-staggered during growing season: Manitoba normal Tuesday with Wednesday holiday/late retry; Saskatchewan Thursday after Tuesday-Monday report period; Alberta Friday by ~1:30 PM MT per official 2026 calendar | `report_date`, `release_date`, `period_start`, `period_end` | Percent seeded/progress/condition/development timing/moisture proxy | PDF/table layout drift, province-specific crop definitions, narrative-only Manitoba crop notes, delayed Alberta release, treating partial Prairie week as complete before Alberta lands; current parser imports seeded progress plus Alberta emergence, Alberta surface-soil moisture/pasture rows, Saskatchewan cropland/hayland/pasture topsoil moisture rows, and Saskatchewan crop-development group rows, but crop-condition scoring remains dormant until official `condition_good_excellent_pct` rows are imported | Canadian seeding and condition context for the premium map and bullish/bearish thesis features where directly mapped. Surfaced in `v_source_freshness` with 10-day strong / 17-day stale-risk thresholds; `grain_market_mappings` admits only V1 Canada lanes with direct official crop definitions (Barley, Canola, Corn, Durum, Oats, Soybeans). Do not use generic Wheat as a proxy for Spring/Winter Wheat. Saskatchewan development group rows can enter thesis packets and score only as low-confidence group-level timing proxies for mapped public grains: Oilseeds -> Canola, Spring Cereals -> Wheat/Durum/Barley/Oats. AB All Crops and SK Cropland `soil_moisture_adequate_surplus_pct` rows can enter thesis packets and score only as low-confidence broad moisture proxies for Canola, Wheat, Durum/Amber Durum, Barley, and Oats; Corn/Soybeans are excluded, and broad moisture rows must not be described as crop-specific condition, quality, or yield. See `docs/reference/canada-crop-progress-release-schedule.md`; Friday Alberta checkpoint is the first safe full-Prairie weekly package. |
| `usda_export_sales` | US / global demand | USDA FAS Export Sales | `usda_export_sales`, `get_wheat_export_history(weeks)` bounded Wheat demand-history RPC | `npm run import-usda` | Weekly Thursday | `week_ending`, `market_year` | Metric tonnes / bushels by source convention; normalize in query layer | Freshness lag, commodity mapping errors | US demand and export pace context. Raw `usda_export_sales` remains authenticated-read; public Wheat visuals use the bounded RPC and do not expose buyer/destination metadata. |
| `usda_wasde` | US / world | USDA WASDE / PSD | `usda_wasde_raw`, `usda_wasde_mapped` | `npm run import-usda-wasde`, archive backfill | Monthly | Report month, marketing year | USDA source units, mapped per attribute | Latest-vs-archive ambiguity, revision history loss | Balance-sheet context for US and world supply/demand. World veg-oil complex admitted 2026-06-09: Oilseed Rapeseed `2226000`, Oil Rapeseed `4239100`, Oil Palm `4243000`, Oil Soybean `4232000` via the PSD `/world/` endpoint (`country_code '00'` — the `/country/00/` path returns empty; world endpoint is latest-snapshot-only, so revision history accrues forward one monthly run at a time and the ESMIS archive cannot backfill it). These rows feed the Canola-only `bounded_context` demand lane (`grain_market_mappings` mapping_type `context`): max +/-6 tilt inside an active CGC demand read, strong `usda_wasde_raw` freshness required (45/75-day monthly thresholds in `get_thesis_data_freshness`), no desk heartbeats. Never primary Canola source truth. |
| `cftc_cot` | Futures positioning | CFTC Commitment of Traders | `cftc_cot_positions` | `python scripts/collect-cftc-cot.py` | Weekly, Friday after CFTC publish for Tuesday report | `report_date` | Contracts; derive tonnes only with explicit contract size | Inherent 3-day lag, source mapping/proxy risk | Portfolio positioning pressure for primary mapped rows only; proxy rows such as soy oil/meal Canola context stay labelled context and do not move deterministic Bull/Bear scores |
| `grain_prices` | Cross-border price context | Current futures/market price feed in app plus Yahoo Finance/Barchart for mapped futures | `grain_prices`, `v_latest_grain_prices`, `get_wheat_price_history(days)` bounded Wheat chart RPC | `npx tsx scripts/import-grain-prices.ts`; run after `python scripts/import-fx-rates.py` for USD/CAD normalization | Weekday market refresh | Sample timestamp / contract date | USD/CAD per bu, tonne, lb, or short ton depending source | Thin market tape, stale weekends/holidays, mapping gaps, FX market holidays | Portfolio price follow-through for mapped contracts: Canola, Wheat, HRW Wheat, Spring Wheat, Corn, Soybeans, Soybean Oil, Soybean Meal, Oats. `get_wheat_price_history` exposes only Spring Wheat/HRW/SRW rows for public `/thesis` visuals; it is confirmation context, not score authority. |
...
| `gee_flood_excess_moisture_watch` | Derived flood / waterlogging watch | Google Earth Engine over Sentinel-1 GRD, GPM IMERG, SMAP L4, NOAA GFS, AAFC ACI, USDA CDL, JRC Global Surface Water, and MERIT Hydro; visual context underlay from AAFC weekly SMOS soil-moisture anomaly and Crop-CASMA SMAP hybrid weekly anomaly for the contiguous U.S. | Event-driven or daily during heavy-rain periods | Watch-only observed surface-water, excess-moisture, forecast-risk, and watch acres over crop masks, displayed as acreage hotspots over official regional soil-moisture anomaly context | V0/V0.1 is admitted only as a labelled watch surface. It must distinguish acres at risk from confirmed damaged acres, remove permanent water, keep observed and forecast layers separate, label AAFC/Crop-CASMA as visual context rather than damage proof, keep public wet/dry field reports in `feedback_log` as calibration/review inputs only, and record freshness in `source_runs`. Current command: `npm run gee:flood-watch -- --event-start YYYY-MM-DD --event-end YYYY-MM-DD`. |
