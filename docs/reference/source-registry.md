...
| `canada_crop_progress` | Canada | Manitoba, Saskatchewan, and Alberta official provincial crop reports | `canada_crop_progress` | `python scripts/import-canada-crop-progress.py` | Weekly during growing season; province-specific release day | `report_date`, `release_date`, `period_start`, `period_end` | Percent seeded/progress/condition | PDF/table layout drift, province-specific crop definitions, narrative-only Manitoba crop notes, delayed Alberta release | Canadian seeding and condition context for the premium map and future bullish/bearish thesis features |
