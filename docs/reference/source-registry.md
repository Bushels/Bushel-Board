...
| `cftc_cot` | Futures positioning | CFTC Commitment of Traders | `cftc_cot_positions` | `python scripts/collect-cftc-cot.py` | Weekly, Friday after CFTC publish for Tuesday report | `report_date` | Contracts; derive tonnes only with explicit contract size | Inherent 3-day lag, source mapping/proxy risk | Positioning pressure where mapping is direct or explicitly proxied |
| `grain_prices` | Cross-border price context | Current futures/market price feed in app plus Yahoo Finance for oilseed proxies | `grain_prices`, `v_latest_grain_prices` | `npx tsx scripts/import-grain-prices.ts`; run after `python scripts/import-fx-rates.py` for USD/CAD normalization | Weekday market refresh | Sample timestamp / contract date | USD/CAD per bu, tonne, or contract depending source | Thin market tape, stale weekends/holidays, mapping gaps, FX market holidays | Price follow-through and scorecard evaluation |
...
| CGC Producer Cars staging import | `cgc_producer_cars` | Thursday after the CGC weekly import | Latest grain week, rows upserted, Canola week cars and crop-year cars total |
| CFTC COT canola positioning import | `cftc_cot` | Friday after CFTC publish | Latest report date, direct Canola managed-money net, proxy rows labelled secondary |
...
| Canola price and FX freshness import | `fx_rates`, `grain_prices` | Weekday evening market refresh | Latest USD/CAD rate, latest Canola settlement, CAD normalization counts |
