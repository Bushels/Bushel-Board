...
**Right now:** Normal `/thesis` is Wheat-first: one farmer-facing Wheat Bull/Bear read, with Canada and U.S. rows treated as evidence geography. CGC import runs through the Codex weekly importer. Wheat price scoring now uses a Spring Wheat / HRW / SRW basket when multiple Wheat futures rows are present, and Wheat weekly-packet headlines now use the deterministic scorecard output instead of driver counting. Active focus: add the Wheat reconciliation judge, finish the USDA Wheat sweep, expose the three-contract price split visually, and polish the relationship spiderweb so source distance/edge weight are instantly readable. **The legacy V1 Grok pipeline (`analyze-grain-market`, `search-x-intelligence`, `analyze-market-data`, `generate-intelligence`, `generate-farm-summary`, and `/api/pipeline/run`) is tombstoned and must not be used as recovery.**
