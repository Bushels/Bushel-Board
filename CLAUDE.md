...
**Right now:** CGC import runs through the Codex weekly importer. 41 feature tracks completed (see `README.md` for compressed log). Active focus: AI quality (Claude Agent Desk = V2 weekly swarm — Claude-only, triggered by Claude Desktop Routines/Schedules), chat alpha, iOS app. **The legacy V1 Grok pipeline (`analyze-grain-market`, `search-x-intelligence`, `analyze-market-data`, `generate-intelligence`, `generate-farm-summary`, and `/api/pipeline/run`) is tombstoned and must not be used as recovery.**
