...
**Right now:** Normal `/thesis` is Wheat-first: one farmer-facing Wheat Bull/Bear read, with Canada and U.S. rows treated as evidence geography (**strategic focus since 2026-06-15: WHEAT-FIRST — hardened to WHEAT-ONLY on 2026-07-12 (Kyle directive, repeated): the weekly desks analyze and publish Wheat ONLY — no LLM tokens on other grains until Kyle explicitly re-expands scope; mechanical collectors unaffected**). CGC import runs through the Codex weekly importer. Wheat price scoring now uses a Spring Wheat / HRW / SRW basket when multiple Wheat futures rows are present, Wheat weekly-packet headlines use the deterministic scorecard output instead of driver counting, and the page now shows a first reconciliation judge, a USDA Wheat source-sweep panel, a node-and-edge relationship spiderweb, and a three-contract price-basket proof strip. GEE satellite crop-stress (`gee_crop_stress`, US HRW + Russia belts, validated vs NASS r=0.93/0.98, `npm run collect:gee-crop-stress` Fri 11 AM MT ahead of the desk swarm) is **watch-only** until multi-season validation. Active focus: deepen the USDA Wheat sweep with live-packet/historical context, improve source-specific reconciliation-judge explanations, and decide whether spiderweb distance should include source authority/freshness/replay accuracy beyond weighted score impact. **The legacy V1 Grok pipeline (`analyze-grain-market`, `search-x-intelligence`, `analyze-market-data`, `generate-intelligence`, `generate-farm-summary`, and `/api/pipeline/run`) is tombstoned and must not be used as recovery.**
...
- `npm run desk:cad -- <cmd>` / `npm run desk:us -- <cmd>` — headless service-role desk CLI (`scripts/desk/desk-cli.ts`), the ONLY Supabase data plane for the Friday desk swarms (Supabase MCP returns -32600 in the scheduled runner). Commands: `preflight` (context + freshness SLA gate; writes `pipeline_runs` failure row + exit 1 on breach), `resolve`, `read --rpc|--table` (allow-listed, select-only), `knowledge` (Viking L2), `write --input <envelope.json>` (zod-validated; dry-run by default; real writes need `--write` + `DESK_WRITE_APPROVAL`), `fail`, `postcheck`.
...
- **V2 (current) — Claude Agent Desk swarm:** The production weekly pipeline. Fully Claude-only, no Grok, no xAI in the analysis loop.
  - **CAD swarm (WHEAT-ONLY since 2026-07-11, reaffirmed firmly by Kyle 2026-07-12):** 6 scouts (supply, demand, basis, sentiment, logistics, macro) → 4 Sonnet specialists (export-analyst, domestic-analyst, risk-analyst, price-analyst) → Opus-class+ desk chief → Opus meta-reviewer — all dispatched for Wheat alone. The 16-grain engine is PARKED for re-enable (parked list + tier budgets retained in the prompt); grain detail/My Farm for parked grains serve their last rows behind `assessDeskThesisStaleness()`. Agent defs: `.claude/agents/{supply,demand,basis,sentiment,logistics,macro}-scout.md` + `.claude/agents/{export,domestic,risk,price}-analyst.md` + `.claude/agents/desk-meta-reviewer.md`. Orchestration prompt: `docs/reference/grain-desk-swarm-prompt.md`. **Wheat is the FLAGSHIP effort tier** (above MAJOR): mandatory Phase 4.5 deep pass, US-desk cross-read (`us_desk_cross_read`, R-CA-WHT-01), wheat-class lens, `llm_metadata.wheat_cockpit` handoff. Triggered by Claude Desktop Routine `grain-desk-weekly` (**Fri 7:45 PM MT — runs AFTER the US desk since 2026-07-11 so the wheat cross-read is same-week**) + Saturday review. Data plane: headless desk CLI (`npm run desk:cad`) — no Supabase MCP. Task ENABLED 2026-07-02 (Kyle-approved) with `DESK_WRITE_APPROVAL` set. Model gate: Opus-class **or above** — a floor, not an allowlist (Fable/Mythos-class passes; never Sonnet/Haiku; verified live 2026-07-12 under Fable 5). Preflight has a one-shot `grain_prices` self-heal (`npm run collect:prices` + single retry when `breached_slas == ["price"]`; added 2026-07-12 after the Jul 10 abort where prices landed 10 minutes after the desk had given up).
  - **US swarm (WHEAT-ONLY as of 2026-07-12, Kyle directive):** US scouts (supply, demand, export, price, cot, wasde, us-conditions, macro; + ag-economy and input-macro cross-market lanes) → 3 stance specialists (export, domestic, price) + risk moderator + planted-area seasonal overlay (Mar 1–Sep 30) → Opus-class+ desk chief → Opus meta-reviewer — one market (Wheat; all three futures/COT classes ZW/KE/MW and WHEAT-SRW/HRW/HRSpring as context), one `us_market_analysis` row per run. Agent defs: `.claude/agents/us-*-scout.md` + `.claude/agents/us-*-analyst.md` + `.claude/agents/us-desk-meta-reviewer.md`. Orchestration prompt: `docs/reference/us-desk-swarm-prompt.md`. Triggered by Claude Desktop Routine `us-desk-weekly` (**Fri 6:47 PM MT — runs FIRST since 2026-07-11**; the CAD FLAGSHIP Wheat read consumes this desk's Wheat stance) + Saturday review. Data plane: headless desk CLI (`npm run desk:us`) — no Supabase MCP. Task re-created and ENABLED 2026-07-12 with Kyle's approval; same Opus-class-or-above floor + price self-heal as the CAD task.
...
- **Daily data collectors (feed V2 scouts + /thesis cache):** Claude Desktop Routines feed Supabase throughout the week: `collect-crop-progress` (Mon), `collect-grain-monitor` (Wed), `collect-export-sales` (Thu AM), `collect-cgc` (Thu PM), `collect-producer-cars` (Thu PM), `collect-prices` (Fri 1:07 PM MT — added 2026-07-12; `grain_prices` has a 4-day desk-preflight SLA but previously NO scheduled collector, which repeatedly aborted the Friday desk), `collect-cftc-cot` (Fri PM), `collect-wasde` (monthly), and `collect-wasde-archive` (monthly revision history). Configs: `docs/reference/collector-task-configs.md`, which also carries the **Routine model + Fable effort matrix (2026-07-13)**: all Routines run Fable-class (never a model downgrade); effort per routine = desks HIGH, collect-grain-monitor MEDIUM (parser self-heal judgment), all other collectors LOW — effort is set in the Claude Desktop Routine settings (the scheduled-task API exposes no model/effort field; each task description carries the contract). **Runner audit 2026-07-12:** `collect-producer-cars` and `collect-sk-prices` were missing from the runner and have been re-created (Thu 4:07 PM / Fri 12:37 PM MT); `collect-wasde-archive` is documented here but still has NO scheduled task — re-create it if the revision-history lane matters this season. Thesis-relevant collectors should use `npm run collect:*` wrappers so successful imports refresh `thesis_packet_cache`. Collectors also write intra-week ticks to `score_trajectory` / `us_score_trajectory` with `scan_type='collector_*'` (Track 45-B).
...
- **Agent debate rules:** `docs/reference/agent-debate-rules.md` — codified rules for continuous improvement of pipeline outputs (flow coherence 1-4, thesis quality 5-8, COT positioning 9-11, price action 12-15, coiled-spring 16-19, **stance accountability 20-21: stance-change ledger + reconciliation table**, grain-specific rules, validation checklist)
...
- Source run freshness: `SELECT source_name, status, latest_source_label, started_at FROM source_runs ORDER BY started_at DESC LIMIT 20;` (schema verified 2026-07-12: `source_runs` has `started_at`/`finished_at`/`created_at` — NO `source_date` or `completed_at` columns)
- Desk preflight failures (fail-loud abort ledger): `SELECT grain_week, status, failure_details->>'reason' AS reason, failure_details->'breached_slas' AS breached, started_at FROM pipeline_runs ORDER BY started_at DESC LIMIT 8;` (`pipeline_runs` has NO `created_at` — order by `started_at`)
...
- **Week contiguity (a "success" import can still have silently skipped a week):** `SELECT generate_series(MIN(grain_week), MAX(grain_week)) AS wk FROM cgc_observations WHERE crop_year='2025-2026' EXCEPT SELECT DISTINCT grain_week FROM cgc_observations WHERE crop_year='2025-2026' ORDER BY wk;` — any rows returned are skipped weeks to backfill. The importer now exits 1 with verdict `MISSING_WEEK_INCIDENT` when `validate-import` flags a gap.
...
- `docs/reference/agent-debate-rules.md` — Codified rules for AI thesis debate moderation (21 rules + grain-specific + checklist)
