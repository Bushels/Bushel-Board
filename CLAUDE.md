...
2. **AI** — Claude/Codex workflows analyze each grain: bull/bear thesis, stance score (-100 to +100), actionable recommendations. Grok/xAI analysis is retired, not a fallback.
...
**Right now:** CGC import runs through the Codex weekly importer. 41 feature tracks completed (see `README.md` for compressed log). **Strategic focus (2026-06-15): WHEAT-FIRST — depth before breadth.** We are perfecting the full bull/bear pipeline on wheat (the most data-rich grain: CGC + WASDE/world + 3 COT classes + both Canada & US + richest international dimension) and validating it against price outcomes *before* deepening the other 15 grains. The weekly desk swarm still publishes all 16 — wheat is the *improvement/showcase* focus, not the only grain served. Two live workstreams: (1) **Stance Model V2** — deterministic relation-overlay rating + bounded international/FX/on-farm-stocks contexts + GEE satellite crop-stress + a stance-change ledger (`docs/plans/2026-06-12-stance-model-v2-design.md`); (2) **Wheat-first site redesign** — data-rich visualizations + mapping + a dedicated data page (`docs/plans/2026-06-15-wheat-first-data-viz-redesign.md`). GEE crop-stress (`gee_crop_stress`, US HRW + Russia belts, validated vs NASS) collects Fri 11am MT ahead of the desk swarm, **watch-only** until multi-season validation. Other active threads: chat alpha, iOS app. **The legacy V1 Grok pipeline (`analyze-grain-market`, `search-x-intelligence`, `analyze-market-data`, `generate-intelligence`, `generate-farm-summary`, and `/api/pipeline/run`) is tombstoned and must not be used as recovery.**
...
- **Intraday signal scanning:** Future work should use the direct X API v2 gateway. Do not revive Grok `x_search`.
