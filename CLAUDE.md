...
- **V2 (current) — Claude Agent Desk swarm:** The production weekly pipeline. Fully Claude-only, no Grok, no xAI in the analysis loop.
  - **CAD swarm:** 6 Haiku scouts (supply, demand, basis, sentiment, logistics, macro) → 3 Sonnet specialists (export-analyst, domestic-analyst, risk-analyst) → Opus desk chief → Opus meta-reviewer. Agent defs: `.claude/agents/{supply,demand,basis,sentiment,logistics,macro}-scout.md` + `.claude/agents/{export,domestic,risk}-analyst.md` + `.claude/agents/desk-meta-reviewer.md`. Orchestration prompt: `docs/reference/grain-desk-swarm-prompt.md`. Triggered by scheduled task `grain-desk-weekly` (Fri 6:47 PM MT) + Saturday review. Data plane: headless desk CLI (`npm run desk:cad`) — no Supabase MCP. Task ENABLED 2026-07-02 (Kyle-approved) with `DESK_WRITE_APPROVAL` set. Model gate: Opus-class **or above** (Fable/Mythos-class passes; never Sonnet/Haiku). Preflight has a one-shot `grain_prices` self-heal (`npm run collect:prices` + single retry when `breached_slas == ["price"]`; added 2026-07-12 after the Jul 10 abort where prices landed 10 minutes after the desk had given up). **WHEAT-ONLY scope as of 2026-07-12 (Kyle directive):** scouts/specialists/chief run for Wheat only; the envelope carries exactly one row (grain='Wheat').
  - **US swarm:** 8 Haiku scouts (supply, demand, export, price, cot, wasde, us-conditions, macro) → 4 Sonnet specialists (export-analyst, domestic-analyst, price-analyst, risk-analyst; planted-area specialist Mar 1–Sep 30) → Opus desk chief → Opus meta-reviewer. Agent defs: `.claude/agents/us-*-scout.md` + `.claude/agents/us-*-analyst.md` + `.claude/agents/us-desk-meta-reviewer.md`. Orchestration prompt: `docs/reference/us-desk-swarm-prompt.md`. Triggered by scheduled task `us-desk-weekly` (Fri 7:30 PM MT) + Saturday review. Data plane: headless desk CLI (`npm run desk:us`) — no Supabase MCP. Task re-created 2026-07-12 (it had been missing from the runner entirely) and ENABLED same day with Kyle's approval; same Opus-class-or-above model gate + price self-heal as the CAD task. **WHEAT-ONLY scope as of 2026-07-12 (Kyle directive):** one market (Wheat, all three futures/COT classes as context), one `us_market_analysis` row per run.
