...
**Right now:** Normal `/thesis` is Wheat-first: one farmer-facing Wheat Bull/Bear read, with Canada and U.S. rows treated as evidence geography. CGC import runs through the Codex weekly importer. Active focus: Wheat relationship scoring, the Wheat price basket, and the visual relationship spiderweb. **The legacy V1 Grok pipeline (`analyze-grain-market`, `search-x-intelligence`, `analyze-market-data`, `generate-intelligence`, `generate-farm-summary`, and `/api/pipeline/run`) is tombstoned and must not be used as recovery.**
