...
| `data-universe-client.tsx` | Thin `next/dynamic` ssr:false wrapper that mounts the scene from the server page. |
| `flood-watch-feedback.tsx` | `/environmental/flood-watch` - public wet/dry field-report intake through `/api/environmental/field-check`, saved into `feedback_log`. Calibration/review only; never auto-promotes watch acres into confirmed damage. |
| `flood-watch-map.tsx` | `/environmental/flood-watch` - watch-only Mapbox map for GEE flood / excess-moisture acreage hotspots over official soil-moisture anomaly context. It separates observed surface-water, excess-moisture, forecast-risk, and watch acres; AAFC/Crop-CASMA underlays are regional context only; never describe any layer as confirmed damaged acres. |
