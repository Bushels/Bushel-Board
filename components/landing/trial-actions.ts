...
export async function submitTrialSignup(input: unknown): Promise<TrialSubmitResult> {
  ...
  if (error) {
    console.error("[trial-actions] submit_bio_trial_signup failed:", error);
...
export async function getEnrolledAcres(): Promise<number> {
  const supabase = await createClient();
  const { data, error } = await supabase.rpc("get_bio_trial_acres");
  if (error) {
    console.warn("[trial-actions] get_bio_trial_acres failed:", error);
    return 0;
  }
