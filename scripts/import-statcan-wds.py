...
Covers two admitted lanes (research-verified live 2026-06-10):
...
def main() -> int:
    ...
    parser.add_argument("--group", choices=["field_crops", "crush", "all"], default="all")
    ...
    groups = ["field_crops", "crush"] if args.group == "all" else [args.group]
    ...
    for group in groups:
        rows = fetch_field_crops(args.periods) if group == "field_crops" else fetch_crush(args.periods)
        ...
        if not args.dry_run:
            ...
            try:
                ...
                log(f"[{group}] source_runs write failed: {exc}")
        sample = rows[:3]
        summary["groups"][group] = {"rows": len(rows), "sample": sample}
